...
default			:	DEFAULT COLON statements BREAK SEMICOL 	{NOTHING
			...
			;
iterativeStmt		:	FOR BO ID{struct value temp;temp.v.s = yylval.sVal;temp.usethis = "identifier";$3=temp; sset=1;FILE *fp = fopen("forloop.txt", "w"); fprintf(fp,"start ");fclose(fp);} IN range BC STARTF statements {FILE *fp; 
fp = fopen("forloop.txt", "a");
fseek(fp,0,SEEK_END);
fprintf(fp," end $");
fclose(fp);} ENDF	{NOTHING
																				temp=$3;
																				char str[20];
																				sprintf(str,"%d",$6->val.v.i);	
			...
			;
range			:	NUM RANGEOP NUM	{NOTHING
				temp.v.i=$1;
				temp.e=$3;
				$$ = create_node(temp,RANGE,NULL,NULL,NULL,NULL);
				}
			; 
%%
#include <stdio.h>
